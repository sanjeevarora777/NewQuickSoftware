...
function AttachEventsForInputMultipleOptions() {
    ...
    .mouseleave(function () {
        ...
        $(this).children().children("input").each(function () {
            if (b != true) {
                b = ($(this).css("background-color") == "#ffe6e6");
            }
        });
        if (b == false) {
            $(this).hide();
...
function CalculateDiscount() {
    ...
    if ($("#discountOption1").attr("value") == "1") {
        var dcp = $("#txtPercentDiscount").val();
        if (dcp != "" && dcp != "0" && isNaN(dcp) == false) {
            netdc = parseFloat(dcp) * parseFloat(discount) / 100;
            $("#discountOption1 span[rel=totalPercentDiscount]").text(netdc);
        }
    }
    if ($("#discountOption2").attr("value") == "1") {
        var dcp = $("#txtFlatDiscount").val();
        if (dcp != "" && dcp != "0" && isNaN(dcp) == false) {
            netdc = dcp;
            $("#discountOption1 span[rel=totalFlatDiscount]").text(netdc);
        }
    }
    $("#txtDiscount").val(netdc);

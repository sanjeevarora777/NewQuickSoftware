...
function ShowLoadingDiv(targetDiv, wrapText, processingText) {
    ...
    if (wrapText == "true") {
        $(targetDiv + " div[name=loading]").append(processingText);
    }
    else {
        $(targetDiv + " div[name=loading]").append('<p>' + processingText + '</p>');
    }
}
...
function CheckValidImage(imageName) {
    var ret = false;
    if (imageName.indexOf(".") > -1 && imageName.lastIndexOf("/") < imageName.indexOf(".")) {
        ret = true;
    }
    return ret;
}
function AddItems(targetDiv, itemName, itemCode, imageName, type, count, targetText, dbid, additionalAttr, dontAttachEvents) {
    var extraText = "";
    ...
    if (imageName != null && imageName != '') {
        ...
        if (CheckValidImage(imageName) == true) {
            t1 = "1";
            $('#' + type + count).append('<img origImg="' + imageName + '" class="itemIconImage" name="' + type + 'Img" src="' + imageFolder + imageName + '"></img>');
        }
    }
    if (t1 == "0") {
        style1 = "style='font-size:14px; font-weight:bold; height:100%;'";
        style2 = "style='white-space:normal;'";
    }
    $('#' + type + count).append('<div class="itemIconName" ' + style1 + ' name="' + type + 'Name"><span class="itemIconNameSpan" ' + style2 + '>' + itemName + '</span></div>');
    $('#' + type + count).append('<span class="itemIconCode" name="code">' + itemCode + '</span>');

    if (dontAttachEvents == "true") { return; }
...
function MarkDiv(divName, imageNameTag, type) {
    $(divName).each(function () {
        ...
        $(this).append('<img class="itemIconCheckImage" src="' + imageFolder + '/checkmark_1.gif" name="' + imageNameTag + '"></img>');
        //var top = $(this).height();
...
function ConvertDate(date) {
    ...
    var d = new Date(date);
    return d_names[d.getDay()] + ' ' + d.getDate() + '-' + (d.getMonth() + 1) + '-' + d.getFullYear();
}
//////////////////////~Common Functions//////////////////////////////////////
...
function LoadProcesses(data, status, xhr, initFlag, targetDiv) {
    ...
    if (IsDivLoaded(targetDiv) == false) {
        if (initFlag == "0") {
            ...
            //var sampleDiv = '<div dbid="@CategoryID"><span>@CategoryName</span></div>';
            for (var i = 0; i < tempIds.length; i++) {
                var isdiscount = GetXmlData(null, "process[id=" + tempIds[i] + "] isdiscount", "nodeText", null, null, xmlData);
                var tax = GetXmlData(null, "process[id=" + tempIds[i] + "] tax", "nodeText", null, null, xmlData);
                AddItems(targetDiv,
                    ...
                    GetXmlData(null, "process[id=" + tempIds[i] + "] Image", "nodeText", null, null, xmlData),
                        "process", i, null, tempIds[i], " isdiscount='" + isdiscount + "' tax='" + tax + "'");
            }
            MarkDivAsLoaded(targetDiv);
        }
    }
...
function AttachEventsForKeyboard() {
    $("input[rel=autoComplete]").focus(function () {
        ...
        var extraFilter = $(this).attr("extraFilter");
        if (extraFilter == null) { extraFilter = ""; }
        if (sourceDiv != null) {
            CreateList(sourceDiv, true, true, this, extraFilter);
        }
...
function SelectListItem() {
    ...
    $(inputid).val('');
    var func = $(inputid).attr("onCustomSelect");
    var fn = window[func];
    var arg1 = $("#list div.selectedListItem").attr("dbid");
    HideList();
    if (arg1 == null) {
        var conf = confirm("The item does not exist. Do you want to add the item?");
    }
    else {
        fn(inputid, arg1);
    }
...
function CreateList(sourceDiv, hasCode, hasIcon, inputTextControl, extraFilter) {
    if (extraFilter == null) { extraFilter = ""; }
    ...
    if (curInput == "txtSearchCustomer") {
        ...
        for (var i = 0; i < tempids.length; i++) {
            $(list).append("<div id= 'listItem" + count + "' seq='" + count + "' class='listItem' dbid='" + tempids[i] + "'></div>");
        ...
        //alert(extraFilter);
        $(sourceDiv + " div.itemIconContainer" + extraFilter).each(function () {
            $(list).append("<div id='listItem" + count + "' seq='" + count + "' class='listItem' dbid='" + $(this).attr("dbid") + "'></div>");
            //Add Code
            if (hasCode == true) {

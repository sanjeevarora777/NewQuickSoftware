...
function UnmarkDiv(divName, imageNameTag, type) {
...
function LoadItems(data, status, xhr, initFlag, targetDiv) {
    ...
    if (IsDivLoaded(targetDiv) == false) {
        if (initFlag == "0") {
            ...
            for (var i = 0; i < tempIds.length; i++) {
                AddItems(targetDiv,
                    GetXmlData(null, "item[id=" + tempIds[i] + "] Name", "nodeText", null, null, xmlData),
                    GetXmlData(null, "item[id=" + tempIds[i] + "] Code", "nodeText", null, null, xmlData),
                    GetXmlData(null, "item[id=" + tempIds[i] + "] Image", "nodeText", null, null, xmlData),
                        "item", i, null, tempIds[i],
                        "catid=" + GetXmlData(null, "item[id=" + tempIds[i] + "]", "attribute", "catid", null, xmlData));
            }
            MarkDivAsLoaded(targetDiv);
...
function AddProcess(moveToLast) {
    if (moveToLast == null) {
        moveToLast = true;
    }
    var dataDiv = "#processData";
    var newProcess = $(dataDiv + " :last-child").clone();
    var newSeq = parseInt($(dataDiv + " :last-child").attr("sequence")) + 1;
    $(dataDiv).append(newProcess);
    SetProcessDataAttributes(dataDiv + " :last-child", "", "", "", newSeq);
    if (moveToLast == true) {
        //Move to New Process
        MoveProcessSelection("last", 0);
    }
}
function UpdateProcess() {
    //Get Current sequence and set all atributes
    var curDiv = "#processData div[sequence=" + $("#processData").attr("current") + "]";
    SetProcessDataAttributes(curDiv, $("#processes div[value=1]").attr("dbid"),
                        $("#txtProcessRate").val(), $("#txtProcessAmount").val(),
                        $(curDiv).attr("sequence"));
}
function DeleteProcess() {
    var dataDiv = "#processData";
    $(dataDiv + " div[sequence=" + $(dataDiv).attr("current") + "]").remove();
    MoveProcessSelection("first", 0);
}
function MoveProcessSelection(flag, curIndex) {
    var dataDiv = "#processData";
    var maxSequence = $(dataDiv + " :last-child").attr("sequence");
    var sequence = 0; //Set to first by default
    UpdateProcess();

    if (flag == "first") { //First
        sequence = 1;
    }
    else if (flag == "last") { //Last
        sequence = maxSequence;
    }
    else if (flag == "next") { //Next
        if (parseInt(curIndex) + 1 <= maxSequence) {
            sequence = parseInt(curIndex) + 1;
        }
    }
    else if (flag == "prev") { //Previous
        if (parseInt(curIndex) > 1) {
            sequence = parseInt(curIndex) - 1;
        }
    }
    if (sequence > 0) {
        ResetProcessControls();
        SelectProcessControls(sequence);
        SetProcessPanelHeaderText(sequence, maxSequence);
    }
}
...
function SelectProcessControls(sequence) {
    if (sequence > 0) {
        var divName = "#processData div[sequence=" + sequence + "]";
        var dbid = $(divName).attr("dbid");
        var rate = $(divName).attr("rate");
        var amount = $(divName).attr("amount");
        MarkDiv("#processes div[dbid=" + dbid + "]", "processCheck", "process");
        $("#txtProcessRate").val(rate);
        $("#txtProcessAmount").val(amount);
        //Set Current attribute
        $("#processData").attr("current", sequence);
    }
}
function SetProcessPanelHeaderText(curIndex, maxIndex) {
    if (curIndex > 0) {
        $("#processPanelHeader span").text("Process " + curIndex + " of " + maxIndex);
    }
}

function AttachEventForPreviousProcess() {
    $("#btnPrevious").click(function () {
        MoveProcessSelection("prev", $("#processData").attr("current"));
    });
}
function AttachEventForNextProcess() {
    $("#btnNext").click(function () {
        MoveProcessSelection("next", $("#processData").attr("current"));
    });
}
...
function LoadNumPad(divName) {
    ...
    $("input:text[rel=numPad]").each(function () {
...
function CloseNumPad(flag) {
    ...
    if (flag == "a") {
        if ($("#" + $("#numPad").attr("txtName")).attr("createRow") == "true") {
            CreateRowFromLineItem();
        }
    }
}
...
function AddNewRow(firstLoad) {
    ...
    $("#bookingTable tbody").append("<tr sequence=" + seq + "><td><span>" + seq + "</span></td><td></td><td></td><td></td><td></td></tr>");
...
function ResetProcesses() {
    ResetProcessControls();
    //Reset Process Data
    $("#processData div[sequence!=1]").remove();
    $("#processData").attr("current", "1");
    SetProcessDataAttributes("#processData div[sequence=1]", "", "", "", "1");
}
function ResetProcessControls() {
    //Unmark all patterns
    UnmarkDiv("#processes div[name=processItems]", "processCheck", "process");
    //Reset all text
    $("#processDesc input:text").val('');
}
//////////////////////~Right Commands//////////////////////////////////////
////////////////////////////Table Events//////////////////////////////////////
...
function CreateRowFromLineItem() {
    ...
    $(tdSel + " td:nth-child(3)").append("<span>" +
                GetItemText(xml) + "-" + GetPatternsText(xml) + "-" + GetColorsText(xml)
             + "</span");
    //Col 4
